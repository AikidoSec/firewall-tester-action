...
psql -h %POSTGRES_HOST% -p %POSTGRES_PORT% -U %POSTGRES_ADMIN_USER% -d postgres -v ON_ERROR_STOP=1 -c "BEGIN; SELECT pg_advisory_xact_lock(hashtext('firewall-tester-action:%POSTGRES_USER%')); DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = '%POSTGRES_USER%') THEN ALTER ROLE %POSTGRES_USER% WITH LOGIN PASSWORD '%POSTGRES_PASSWORD%'; ELSE CREATE ROLE %POSTGRES_USER% WITH LOGIN PASSWORD '%POSTGRES_PASSWORD%'; END IF; END $$; COMMIT;"
if errorlevel 1 (
